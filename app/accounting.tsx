...
type WeekStat={label:string;income:number;expenses:number;profit:number;operations:number;units:number};
...
export function AccountingPanel({txs,products,services}:{txs:Tx[];products:Product[];services:ServiceLike[]}){
  ...
  while(cursor.getTime()<=period.end.getTime()&&index<=54){const candidate=addDays(cursor,6),weekEnd=endDay(candidate.getTime()>period.end.getTime()?period.end:candidate),weekRows=dated.filter(item=>inRange(item.date,cursor,weekEnd)).map(item=>item.tx).filter(validTx),weekIncome=sum(weekRows.filter(paidIncome)),weekExpenses=sum(weekRows.filter(tx=>tx.type==="Egreso"));weeks.push({label:`Semana ${index}`,income:weekIncome,expenses:weekExpenses,profit:weekIncome-weekExpenses,operations:weekRows.length,units:weekRows.filter(tx=>tx.origin==="product-sale"&&paidIncome(tx)).reduce((total,tx)=>total+(Number(tx.quantity)||Math.abs(Number(tx.stockDelta))||0),0)});cursor=startDay(addDays(weekEnd,1));index++}
  const bestIncome=[...weeks].sort((a,b)=>b.income-a.income)[0],bestProfit=[...weeks].sort((a,b)=>b.profit-a.profit)[0],mostOperations=[...weeks].sort((a,b)=>b.operations-a.operations)[0],maxWeek=Math.max(1,...weeks.map(week=>Math.max(week.income,week.expenses))),avgWeeks=Math.max(1,weeks.length);
  ...
  return <div style={{display:"grid",gap:18}}>
    ...
    <section className="panel"><Heading title="Evolución semanal" text="Ingresos, egresos, utilidad y actividad por semana"/><div style={{display:"grid",gap:12}}>{weeks.map(week=><div key={week.label} style={{display:"grid",gap:7,paddingBottom:12,borderBottom:"1px solid #edf0ee"}}><div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}><b style={{color:"#244d45"}}>{week.label}</b><small style={{color:"#7d8581"}}>{week.operations} movimientos · {week.units} unidades</small></div><Bar label="Ingresos" amount={week.income} max={maxWeek}/><Bar label="Egresos" amount={week.expenses} max={maxWeek} expense/><div style={{textAlign:"right",fontSize:12,color:"#6d7773"}}>Utilidad <b style={{color:"#244d45"}}>{money(week.profit)}</b></div></div>)}</div><div style={{...cardGrid,marginTop:14}}><Mini label="Semana con mayor ingreso" value={bestIncome?.label||"Sin datos"} note={bestIncome?money(bestIncome.income):undefined}/><Mini label="Semana con más movimientos" value={mostOperations?.label||"Sin datos"} note={mostOperations?`${mostOperations.operations} movimientos`:undefined}/><Mini label="Mejor semana por utilidad" value={bestProfit?.label||"Sin datos"} note={bestProfit?money(bestProfit.profit):undefined}/></div><div style={{...cardGrid,marginTop:12}}><Mini label="Ingreso promedio semanal" value={money(income/avgWeeks)}/><Mini label="Egreso promedio semanal" value={money(expenses/avgWeeks)}/><Mini label="Utilidad promedio semanal" value={money(profit/avgWeeks)}/></div></section>
...
function Bar({label,amount,max,expense=false}:{label:string;amount:number;max:number;expense?:boolean}){return <div style={{display:"grid",gridTemplateColumns:"65px minmax(40px,1fr) 90px",gap:8,alignItems:"center",fontSize:11,color:"#6f7774"}}><span>{label}</span><div style={{height:8,borderRadius:999,background:"#edf0ee",overflow:"hidden"}}><div style={{height:"100%",width:`${amount/max*100}%`,background:expense?"#b59a5a":"#2f6b5e",borderRadius:999}}/></div><b style={{textAlign:"right",color:"#344b46"}}>{money(amount)}</b></div>}

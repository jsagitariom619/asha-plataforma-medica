...
async function authenticatedCaller(token: string): Promise<Caller | null> {
  ...
  const authUser = await readJsonSafe(authResponse),
    id = typeof authUser.id === "string" ? authUser.id : "";
  if (!id) return null;
  ...
  const rows = permissionResponse.ok
...
function canUseField(caller: Caller, field: string) {
  return caller.isPrimaryAdmin
    ? field in FIELD_MODULES
    : (FIELD_MODULES[field] || []).some((module) =>
        caller.permissions.has(module),
      );
}
function visibleState(state: State, caller: Caller) {
  const visible = Object.fromEntries(
    Object.entries(state).filter(([field]) => canUseField(caller, field)),
  );
    ...
    visible.clinicalHistories = visible.clinicalHistories.map((record) => {
      ...
      if (data && typeof data === "object" && !Array.isArray(data)) {
        for (const field of ["beforePhoto", "afterPhoto"]) {
          ...
          if (typeof value === "string" && value.startsWith("clinical-storage:")) {
            (data as State)[field] = `/api/clinical-photo?path=${encodeURIComponent(value.slice(17))}`;
          }
        }
      }
      return copy;
...
async function uploadImage(bucket: string, path: string, value: unknown) {
  ...
  const response = await supabaseAdminFetch(
    `/storage/v1/object/${bucket}/${path}.${image.extension}`,
    { method: "POST", headers: { "Content-Type": image.mime, "x-upsert": "true" }, body: image.bytes },
  );
...
async function persistImages(state: State) {
  ...
  if (Array.isArray(next.products)) {
    next.products = await Promise.all(next.products.map(async (product) => {
      ...
      const copy = product as State, id = String(copy.id || crypto.randomUUID());
  ...
  if (Array.isArray(next.clinicalHistories)) {
    next.clinicalHistories = await Promise.all(next.clinicalHistories.map(async (record) => {
      if (!record || typeof record !== "object") return record;
      const copy = record as State, id = String(copy.id || crypto.randomUUID()), data = copy.data;
      if (data && typeof data === "object" && !Array.isArray(data)) {
        for (const field of ["beforePhoto", "afterPhoto"]) {
...
async function readCentralState(): Promise<{
  payload: State;
  revision: number;
  updated_at: string | null;
} | null> {
  const response = await supabaseAdminFetch(
    "/rest/v1/clinic_state?select=payload,revision,updated_at&id=eq.asha&limit=1",
    { headers: { Accept: "application/json" } },
  );
  if (!response.ok)
    throw new Error(`clinic_state read failed: ${response.status}`);
...
export async function GET(request: Request) {
  try {
    const caller = await authenticatedCaller(
      bearerToken(request) || (await readAccessToken()) || "",
    );
    ...
    return NextResponse.json(
      {
        ok: true,
        state: row ? visibleState(row.payload, caller) : null,
...
export async function PUT(request: Request) {
  try {
    ...
    if (!raw || Buffer.byteLength(raw, "utf8") > MAX_PAYLOAD_BYTES)
      return fail("La información excede el tamaño permitido.", 413);
    let body: { state?: unknown };
    try {
      body = JSON.parse(raw) as { state?: unknown };
    } catch {
      return fail("Contenido no válido.", 400);
    }
    if (
      !body.state ||
    ...
    const permitted = Object.fromEntries(
      Object.entries(body.state as State).filter(([field]) =>
        canUseField(caller, field),
      ),
    );
    if (!Object.keys(permitted).length)
      return fail("No tienes permiso para guardar esta información.", 403);
    const persisted = await persistImages(permitted),
      current = await readCentralState(),
      payload = { ...(current?.payload || {}), ...persisted },
      revision = (current?.revision || 0) + 1;
    const response = await supabaseAdminFetch(
      "/rest/v1/clinic_state?on_conflict=id",
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      console.error(
        "ASHA clinic state write rejected",
        response.status,
        await response.text(),
      );
    ...
    return NextResponse.json(
      {
        ok: true,
        revision: rows[0]?.revision ?? revision,

...
export function PatientBillingPanel({patients,attentions,txs,onPayment,onNewCharge}:{patients:Patient[];attentions:Attention[];txs:Tx[];onPayment:(patientId:number,attentionId:number,amount:number,method:string)=>string;onNewCharge?:(patientId?:number)=>void}){
  ...
  const newCharge=(patientId?:number)=>{if(onNewCharge){onNewCharge(patientId);return}setNotice("La selección del paciente ya está preparada. El cobro nuevo se conectará al registro financiero central en la siguiente integración.")};
  ...
  return <>
    <section className="panel" style={{display:"grid",gap:14}}>
      <div className="title" style={{marginBottom:0,alignItems:"center"}}><div><h3>Estado de cuenta por paciente</h3><p>Busca cualquier paciente registrado para cobrar, revisar pagos o registrar un abono.</p></div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span className="tag" style={{fontSize:10}}>{money(pendingTotal)} pendiente</span><Button className="gold" type="button" onClick={()=>newCharge()}><Plus/>Nuevo cobro</Button></div></div>
      <div className="search" style={{marginBottom:0,maxWidth:620}}><Search/><Input value={query} onChange={event=>{setQuery(event.target.value);setNotice("")}} placeholder="Buscar paciente por nombre, historia o teléfono"/></div>
      {notice&&<p className="form-note" role="status" style={{margin:0}}>{notice}</p>}
      {clean&&<div style={{display:"grid",gap:8}}>{summaries.length?summaries.map(summary=>{const pending=summary.rows.find(row=>row.balance>0);return <div key={summary.patient.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",border:"1px solid #e6ebe8",borderRadius:12,background:"#fff",flexWrap:"wrap"}}><span className="avatar"><UserRound/></span><div style={{minWidth:180,flex:1}}><b style={{display:"block"}}>{summary.patient.name}</b><small style={{display:"block",color:"#7a8581"}}>{summary.patient.code} · {summary.patient.phone}</small></div><div style={{display:"grid",gap:2,minWidth:120}}><small style={{color:"#7a8581"}}>Saldo</small><b>{money(summary.balance)}</b></div>{pending?<Button type="button" variant="outline" onClick={()=>setSelected(pending)}>Registrar abono</Button>:<Button type="button" variant="outline" onClick={()=>newCharge(summary.patient.id)}><Plus/>Nuevo cobro</Button>}</div>}):<div style={{padding:"18px",textAlign:"center",color:"#7d8581",fontSize:12}}>No se encontró ningún paciente con ese nombre, historia o teléfono.</div>}</div>}

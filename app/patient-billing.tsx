...
export function PatientBillingPanel({patients,attentions,txs,onPayment}:{patients:Patient[];attentions:Attention[];txs:Tx[];onPayment:(patientId:number,attentionId:number,amount:number,method:string)=>string}){
  ...
  return <>
    <section className="panel" style={{display:"grid",gap:14}}>
      ...
      {clean&&<div style={{display:"grid",gap:8}}>{summaries.length?summaries.map(summary=>{const pending=summary.rows.find(row=>row.balance>0);return <div key={summary.patient.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 14px",border:"1px solid #e6ebe8",borderRadius:12,background:"#fff",flexWrap:"wrap"}}><span className="avatar"><UserRound/></span><div style={{minWidth:180,flex:1}}><b style={{display:"block"}}>{summary.patient.name}</b><small style={{display:"block",color:"#7a8581"}}>{summary.patient.code} · {summary.patient.phone}</small></div><div style={{display:"grid",gap:2,minWidth:120}}><small style={{color:"#7a8581"}}>Saldo</small><b>{money(summary.balance)}</b></div>{pending?<Button type="button" variant="outline" onClick={()=>setSelected(pending)}>Registrar abono</Button>:<Button type="button" variant="outline" onClick={()=>setNewChargePatientId(summary.patient.id)}><Plus/>Nuevo cobro</Button>}</div>}):<div style={{padding:"18px",textAlign:"center",color:"#7d8581",fontSize:12}}>No se encontró ningún paciente con ese nombre, historia o teléfono.</div>}</div>}

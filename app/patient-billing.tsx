...
const compactActionStyle={width:126,height:34,minHeight:34,padding:"0 10px",fontSize:12,justifyContent:"center",whiteSpace:"nowrap" as const};
...
function saveDirectPayment(row:BalanceRow,amount:number,method:string){
  ...
  const store=readStore(),existing:Tx[]=Array.isArray(store.txs)?store.txs:[],pending=existing.find(tx=>tx.operationId===row.operationId&&tx.origin==="patient-charge"&&tx.status==="Pendiente"),balance=Number(pending?.amount)||0;
  if(!pending||amount<=0||amount>balance+0.001)return "El importe supera el saldo pendiente.";
  const createdAt=new Date().toISOString(),stamp=Date.now(),payment:Tx={id:stamp,concept:row.concept,reference:row.patient.name,type:"Ingreso",amount,date:nowLabel(),createdAt,method,status:"Pagado",origin:"patient-payment",operationId:row.operationId,patientId:row.patient.id};
  const next=existing.flatMap(tx=>{if(tx.id!==pending.id)return[tx];const remaining=Math.max(0,(Number(tx.amount)||0)-amount);return remaining>0?[{...tx,amount:remaining,date:nowLabel(),createdAt}]:[]});
  localStorage.setItem("asha-demo",JSON.stringify({...store,txs:[payment,...next]}));
  return "";
}
...
export function PatientBillingPanel({patients,attentions,txs,onPayment}:{patients:Patient[];attentions:Attention[];txs:Tx[];onPayment:(patientId:number,attentionId:number,amount:number,method:string)=>string}){
  ...
  return <>
    <section className="panel" style={{display:"grid",gap:14}}>
      ...
      {clean&&<div style={{display:"grid",gap:6}}>{summaries.length?summaries.map(summary=>{const pending=summary.rows.find(row=>row.balance>0);return <div key={summary.patient.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",minHeight:52,border:"1px solid #e6ebe8",borderRadius:10,background:"#fff",flexWrap:"wrap"}}><span className="avatar" style={{width:34,height:34,minWidth:34}}><UserRound style={{width:18,height:18}}/></span><div style={{minWidth:180,flex:1,lineHeight:1.25}}><b style={{display:"block",fontSize:13,fontWeight:600}}>{summary.patient.name}</b><small style={{display:"block",fontSize:11,color:"#7a8581",marginTop:2}}>{summary.patient.code} · {summary.patient.phone}</small></div><div style={{display:"grid",gap:1,minWidth:90,lineHeight:1.2}}><small style={{fontSize:10,color:"#7a8581"}}>Saldo</small><b style={{fontSize:12,fontWeight:600}}>{money(summary.balance)}</b></div>{pending?<Button type="button" variant="outline" style={compactActionStyle} onClick={()=>setSelected(pending)}>Registrar abono</Button>:<Button type="button" variant="outline" style={compactActionStyle} onClick={()=>setNewChargePatientId(summary.patient.id)}><Plus/>Nuevo cobro</Button>}</div>}):<div style={{padding:"14px",textAlign:"center",color:"#7d8581",fontSize:12}}>No se encontró ningún paciente con ese nombre, historia o teléfono.</div>}</div>}
      {visibleRows.length?<div className="table"><table><thead><tr><th>Paciente</th><th>Concepto</th><th>Total</th><th>Pagado</th><th>Saldo</th><th></th></tr></thead><tbody>{visibleRows.map(row=><tr key={row.attention?.id||row.operationId}><td><b>{row.patient.name}<small>{row.patient.code}</small></b></td><td>{row.concept}</td><td>{money(row.total)}</td><td className="green">{money(row.paid)}</td><td className={row.balance>0?"red":"green"}>{money(row.balance)}</td><td>{row.balance>0?<Button type="button" variant="outline" style={compactActionStyle} onClick={()=>setSelected(row)}>Registrar abono</Button>:<span className="tag">Pagado</span>}</td></tr>)}</tbody></table></div>:!clean?<div style={{padding:"18px",textAlign:"center",color:"#7d8581",fontSize:12}}>No hay cobros todavía. Usa “Nuevo cobro” o busca un paciente registrado.</div>:null}
    </section>
...
function PaymentDialog({row,close,onPayment}:{row:BalanceRow|null;close:()=>void;onPayment:(patientId:number,attentionId:number,amount:number,method:string)=>string}){
  const[error,setError]=useState("");
  if(!row)return null;
  const submit=(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();setError("");const data=new FormData(event.currentTarget),amount=Number(data.get("amount")),method=String(data.get("method")||"Efectivo");if(!Number.isFinite(amount)||amount<=0){setError("Ingresa un importe válido.");return}if(amount>row.balance+0.001){setError(`El abono no puede superar el saldo de ${money(row.balance)}.`);return}if(row.direct){const message=saveDirectPayment(row,amount,method);if(message){setError(message);return}close();window.location.reload();return}if(!row.attention){setError("No se encontró la atención asociada.");return}const message=onPayment(row.patient.id,row.attention.id,amount,method);if(message){setError(message);return}close()};
  return <Dialog open onOpenChange={open=>!open&&close()}><DialogContent><DialogHeader><DialogTitle>Registrar abono</DialogTitle><DialogDescription>{row.patient.name} · {row.concept}</DialogDescription></DialogHeader><form className="form" onSubmit={submit}><div className="metrics" style={{gridTemplateColumns:"repeat(3,1fr)"}}><Mini icon={<CircleDollarSign/>} label="Total" value={money(row.total)}/><Mini icon={<WalletCards/>} label="Pagado" value={money(row.paid)}/><Mini icon={<WalletCards/>} label="Saldo" value={money(row.balance)}/></div><label className="field"><Label>Importe del abono</Label><Input name="amount" type="number" inputMode="decimal" min="0.01" max={row.balance} step="0.01" defaultValue={row.balance} required/></label><label className="field"><Label>Método de pago</Label><select name="method" defaultValue="Efectivo"><option>Efectivo</option><option>QR</option><option>Transferencia</option><option>Tarjeta</option><option>Otro</option></select></label>{error&&<p className="auth-error" role="alert">{error}</p>}<div className="form-actions"><Button type="button" variant="outline" onClick={close}>Cancelar</Button><Button className="gold" type="submit">Registrar abono</Button></div></form></DialogContent></Dialog>
}

...
function getPinPepper(): string {
  ...
  if (!value || value.length < 32) throw new Error("Missing or weak ASHA_PIN_PEPPER");
  return value;
}

export function deriveInternalPassword(username: string, pin: string): string {
  const normalized = normalizeUsername(username);
  return createHmac("sha256", getPinPepper()).update(`asha:${normalized}:${pin}`).digest("hex");
}
...
export function initialsFromName(value: string): string {
  return value.trim().split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "US";
}

export const ASHA_MODULES = [
  "Resumen", "Pacientes", "Historias clínicas", "Agenda", "Servicios", "Productos",
  "Caja y cobros", "Movimientos", "Contabilidad", "Usuarios", "Configuración",
] as const;

export function sanitizePermissions(value: unknown): string[] {
  if (!Array.isArray(value)) return [];
  const allowed = new Set<string>(ASHA_MODULES);
  return Array.from(new Set(value.filter((item): item is string => typeof item === "string").map((item) => item.trim()).filter((item) => allowed.has(item))));
}
